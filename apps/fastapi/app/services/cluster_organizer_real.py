...
_DATA_DIR = Path(os.getenv("DATA_DIR", str(_FASTAPI_ROOT / "data")))
_DEFAULT_CLUSTERS_PATH = Path(
    os.getenv("CLUSTERS_PATH", _DATA_DIR / "cluster_output" / "clusters.json")
    )


def get_clusters(path: Path = _DEFAULT_CLUSTERS_PATH) -> List[Dict[str, Any]]:
    """Load and return the clusters list from JSON (reads every call)."""
    if not path.exists():
        raise FileNotFoundError(
            f"clusters.json not found at {path}.\n"
            "Generate it with the clustering script or set CLUSTERS_PATH/DATA_DIR."
        )
...
def _files_by_cluster_for_sources(sources: List[str], clusters: List[Dict[str, Any]]) -> Dict[str, List[str]]:
...
def get_view_text_by_cluster() -> Dict[str, Dict[str, List[str]]]:
    ...
    per_view_files = {
        view: _files_by_cluster_for_sources(VIEW_TO_SOURCES[view], clusters)
